...
class STTController:
    ...
    def __init__(
        ...
        self.vad = SileroVAD(threshold=0.6, silence_threshold=silence_threshold, frame_size=frame_size, verbose=verbose)
    ...
    def _process_audio_buffer(self):
        ...
        while True:
            try:
                audio_packet = self.buffer.get(self.frame_size, timeout=-1)
        ...
        for audio_packet in audio_packets:
            ...
            if is_speech:
                ...
                if not self.caught_voice:
                    ...
                    logger.success(f'caught voice for the first time at {audio_packet.timestamp}') 
            else:
                # print('Silence')
