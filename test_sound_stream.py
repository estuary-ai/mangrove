...
if __name__ == '__main__':
    ...
    parser.add_argument('-q', '--buffersize', type=int, default=100, help='number of blocks used for buffering (default: %(default)s)')
    ...
    try:
        ...
        while True:
            ...
            def _process_audio_buffer():
                ...
                while True:
                    ...
                    if data_buffer.empty() and not recording_thread.is_alive():
                        logger.warning('No more audio packets in buffer, and recording thread is not alive anymore')
                        break
            ...
            transcription = listening_stream.stt_controller._finish_stream(force_clear_buffer=True)
            if transcription is not None:
