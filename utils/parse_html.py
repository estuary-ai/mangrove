...
def extract_dialogue(soup):
    ...
    for font in fonts:
        s = "".join(
            [
                l
                for l in str(font.find_next().nextSibling).strip()
                if (l not in string.punctuation or l == "'") and not l.isdigit()
            ]
        ).upper()
        if s.strip():
            res.append(s.strip())
...
if __name__ == "__main__":
    ...
    append_data(
        "data/librispeech-lm-norm.txt", "data/librispeech-lm-norm_updated.txt", dialogue
    )
